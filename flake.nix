{
  ...
  nixConfig = {
    extra-substituters = [ "https://authentik-nix-fpletz.cachix.org" ];
            ...
            lib.mkAuthentikScope =
              ...
              import ./components {
                inherit
                  ...
                  buildNapalmPackage
                  ;
                authentik-src = pkgs.stdenv.mkDerivation {
                  name = "authentik-src";
                  src = authentik-src;
                  patches = [
                    (pkgs.fetchpatch {
                      url = "https://patch-diff.githubusercontent.com/raw/goauthentik/authentik/pull/9516.patch";
                      hash = "sha256-HJurEdSrTzMv9pHqW6v+S07QsB8mGro+BVlCPcN/VAE=";
                    })
                  ];
                  dontConfigure = true;
                  dontBuild = true;
